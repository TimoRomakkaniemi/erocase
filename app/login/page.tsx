...
type Step = 'email' | 'otp' | 'checkEmail'
...
function LoginForm() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const redirect = `${window.location.origin}/auth/callback?next=${encodeURIComponent(nextUrl)}`
              ...
              {step === 'checkEmail'
                ...
                : step === 'email'
                  ? authMode === 'signup'
                    ...
                    : (t('auth.subtitle'))
                  : (t('auth.otpSubtitle').replace('{email}', email))}
            </p>
          </div>
          ...
          {step === 'checkEmail' ? (
            <div className="space-y-4 text-center">
              <button
                ...
                className="w-full h-11 rounded-xl text-sm font-semibold text-white transition-all hover:brightness-110"
            ...
            <div className="space-y-4">
              <div className="flex rounded-xl p-0.5 gap-0.5" style={{ background: 'rgba(255,255,255,0.06)' }}>
                <button
                  type="button"
                  onClick={() => { setAuthMode('signin'); setError(''); setConfirmPassword('') }}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${authMode === 'signin' ? 'text-white bg-white/15' : 'text-gray-400 hover:text-gray-200'}`}
                >
                  {t('auth.tabSignIn')}
                </button>
                <button
                  type="button"
                  onClick={() => { setAuthMode('signup'); setError(''); setConfirmPassword('') }}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${authMode === 'signup' ? 'text-white bg-white/15' : 'text-gray-400 hover:text-gray-200'}`}
                >
                  {t('auth.tabSignUp')}
                </button>
              </div>
              ...
              {authMode === 'signup' ? (
                <form onSubmit={handleSignUp} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading || !email.trim()}
                    ...
                    style={{ background: 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)', boxShadow: '0 4px 12px rgba(34,197,94,0.3)' }}>
                    {loading ? t('auth.sending') : t('auth.signUpSubmit')}
                  </button>
                </form>
              ) : (
              ...
              {authMode === 'signin' && (
                <p className="text-center text-xs text-gray-500">{t('auth.magicLinkHint')}</p>
              )}
              {authMode === 'signup' && (
                <p className="text-center text-xs text-gray-500">{t('auth.signUpMagicHint')}</p>
              )}
            </div>
          ) : (
            <div className="space-y-4">
